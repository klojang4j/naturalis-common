...
/**
 * Defines ways to increase the capacity of data structures that automatically grow
 * as they fill up (for example list-like data structures).
 *
...
public enum ExpansionType {
  /**
   * Inxrease the capacity by a fixed amount each time the data structure reaches
   * full capacity.
   */
  ADD,
  /**
  ...
  /**
   * Augments the specified curCapacity by the specified amount using {@code this}
   * augmentation type with a minimum amount of 1.
   *
  ...
  /**
   * Augments the specified curCapacity by the specified amount using {@code this}
   * augmentation type with a minimum amount of {@code minAmount}. This method will
   * throw an {@link IllegalArgumentException} if the increased curCapacity goes
   * beyond {@link Integer#MAX_VALUE} or {@link Integer#MIN_VALUE}.
   *
   * @param curCapacity The current capacity
   * @param amount The amount by which to increase it
  ...
  public int increaseCapacity(int curCapacity, double amount, int minAmount) {
    double newCapacity = increaseCapacity((double) curCapacity,
        amount,
        (double) minAmount);
    if (newCapacity < curCapacity + 1 || newCapacity > Integer.MAX_VALUE) {
      throw new IllegalArgumentException("Invalid value for new capacity: "
          + newCapacity);
    }
  ...
  /**
   * Augments the specified curCapacity by the specified amount using {@code this}
   * augmentation type with a minimum amount of {@code minAmount}.
   *
   * @param curCapacity The current capacity
   * @param amount The amount by which to increase it
   ...
   * @return The value to used for the new capacity
   */
  public double increaseCapacity(double curCapacity,
      double amount,
      double minAmount) {
    switch (this) {
      case ADD:
      ...
      case PERCENTAGE:
      default:
        return Math.max(curCapacity * ((100 + amount) / 100),
            curCapacity + minAmount);
    }
  }
}
