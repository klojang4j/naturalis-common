...
public class PrintMethods {
    ...
    try {
      if (map == null) {
        ...
        PrintStream ps = (out instanceof PrintStream)
            ? (PrintStream) out
            : new PrintStream(out);
        Map<String, String> copy = new TreeMap<>((o1, o2) -> toStr(o1).compareTo(
            toStr(o2)));
        map.entrySet()
            .forEach(e -> copy.put(toStr(e.getKey()), toStr(e.getValue())));
        int maxlen = copy.keySet()
            .stream()
    ...
    try {
      if (obj == null) {
        ...
        PrintStream ps = (out instanceof PrintStream)
            ? (PrintStream) out
            : new PrintStream(out);
        Map<String, Getter> getters = GetterFactory.INSTANCE.getGetters(obj.getClass(),
            true);
        int maxlen = getters.keySet()
            .stream()
            .mapToInt(String::length)
            .max()
            .getAsInt();
  ...
  /**
   * Returns a human-friendly representation of the duration between the specified
   * start and now. Example: 540:00:12.630
   *
   * @param start The start time
   * @return A human-friendly representation of the duration between the specified
   *     start and now
   */
  ...
  /**
   * Returns a human-friendly representation of the duration of the specified time
   * interval. Example: 00:08:07.041
   *
   * @param start The start time
   * @param end The end time
   * @return A human-friendly representation of the duration of the specified time
   *     interval
   */
  ...
  /**
   * Returns a human-friendly representation of the specified duration in
   * milliseconds.
   *
   * @param millis The duration in millisecond
   * @return A human-friendly representation of the specified duration in
   *     milliseconds
   */
  public static String duration(long millis) {
    ...
    long s = millis / 1000;
    millis %= 1000;
    return append(new StringBuilder(12),
        lpad(h, 2, '0', ":"),
        lpad(m, 2, '0', ":"),
        lpad(s, 2, '0', "."),
        lpad(millis, 3, '0')).toString();
  }
