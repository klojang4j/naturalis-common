...
class Messages {
    ...
    static Formatter msgDeepNotNull() {
        return args -> {
            ...
            return format(
                    "%s must not be null or contain null values (was %s)", args.argName(),
                    toStr(args.arg()));
        };
    }
    ...
    static Formatter msgEmpty() {
        return args -> {
            if (args.negated()) {
                return format("%s must not be null or empty (was %s)", args.argName(),
                              toStr(args.arg()));
            }
    ...
    static Formatter msgBlank() {
        return args -> {
            if (args.negated()) {
                return format("%s must not be null or blank (was %s)", args.argName(),
                              toStr(args.arg()));
            }
    ...
    static Formatter msgSameAs() {
        return args -> {
            if (args.negated()) {
                ...
                String id0 = className(args.object()) + '@' + System.identityHashCode(
                        args.object());
    ...
    private static String mapToString(Map m) {
        ...
        String sep = DEFAULT_IMPLODE_SEPARATOR;
        String imploded = trim(implode(m.entrySet(), Messages::entryToString, sep, 0, 10),
                               m.size());
        return new StringBuilder(32)
                .append(scn)
                .append(" of {")
