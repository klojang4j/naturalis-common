...
public final class ObjectCheck<T, E extends Exception> {
   ...
   */
  public <P, F extends Throwable> P ok(ThrowingFunction<T, P, F> transformer)
      throws F {
    return transformer.apply(arg);
  }

  /**
   * Passes the validated argument to the specified {@code Consumer}. To be used as
   ...
   * @throws E If the argument is invalid
   */
  public ObjectCheck<T, E> is(ObjIntRelation<T> test,
      int object,
      String message,
      Object... msgArgs) throws E {
    if (test.exists(arg, object)) {
   ...
   * attention when providing a lambda or method reference for <b>both</b> the {@code
   * property} argument <b>and</b> the {@code test} argument. Plain lambdas or method
   * references will cause the compiler to complain about an <b>Ambiguous method
   * call</b>. (If the {@code property} argument is a getter from the {@code
   * CommonGetters} class or the {@code test} argument is a check from the {@code
