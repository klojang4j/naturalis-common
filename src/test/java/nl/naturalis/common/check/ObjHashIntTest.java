...
public class ObjHashIntTest {
  ...
  public void vanilla00() throws IOException {
    ...
    Check.that(c, "collection").has(size(), lt(), 10, "Size must be ${test} ${0}", "ten");
    ...
    Check.that(c, "collection").has(size(), multipleOf(), 3, () -> new IOException());
  ...
  public void vanilla01() throws IOException {
    ...
    Check.that(c, "collection").notHas(size(), gt(), 10, "Size must be ${test} ${0}", "ten");
  ...
  public void has_IntObjRelation00() {
    ...
    try {
      Check.that(c, "buffy").has(listSize(), inRange(), IntPair.of(100, 200));
    } catch (IllegalArgumentException e) {
      System.out.println(e.getMessage());
  ...
  public void notHas_IntObjRelation00() {
    List<String> c = List.of("a", "b", "c", "d", "e", "f");
    try {
      Check.that(c, "buffy").notHas(listSize(), inRange(), IntPair.of(0, 10));
    } catch (IllegalArgumentException e) {
      System.out.println(e.getMessage());
  ...
  public void lambdasTest() {
    ...
    Check.that(c).has(toInt(Collection::size), intInt((x, y) -> x < y), 10);
    Check.that(c).has(Collection::size, intInt((x, y) -> x < y), 10);
    Check.that(c).has(x -> x.size(), intInt((x, y) -> x < y), 10);
    Check.that(c).has(toInt(x -> x.size()), x -> x < 10);
  }
}
